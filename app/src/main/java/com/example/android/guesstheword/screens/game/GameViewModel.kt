...
class GameViewModel : ViewModel() {
    ...
    companion object {
        ...
        const val COUNTDOWN_PANIC_SECONDS = 10000L
    }
    ...
    // Checking game finished state
    private val _gameFinished = MutableLiveData<Boolean>(false)
    val gameFinished: LiveData<Boolean> get() = _gameFinished

    // The different buzzing events
    private val _buzzEvent = MutableLiveData<BuzzType>(BuzzType.NO_BUZZ)
    ...
    init {
        Log.i("GameViewModel", "an instance of GameViewModel has been created.")
        resetList()
        nextWord()
        ...
        timer = object : CountDownTimer(COUNTDOWN_TIME, ONE_SECOND) {
            ...
            override fun onTick(millsUntilFinished: Long) {
                if(millsUntilFinished <= COUNTDOWN_PANIC_SECONDS){
    ...
    override fun onCleared() {
        super.onCleared()
        timer.cancel()
        Log.i("GameViewModel", "onCleared() of GameViewModel called.")
